<template>
    <div class="city-warp">
        ...
        <Scroll class="scroll" v-show="!keyword" ref="scroll" :listenScroll='listenScroll' :probeType='probeType' @scroll='scroll'>
            <div>
        ...
        <Scroll :data='cityList' class="search-content" v-show="keyword" ref="search"> 
            <ul>
                <li v-for="(val,index) in cityList" :key="index"  @click="selectCiti(val.name)" class="border-bottom">{{val.name}}</li>
            </ul>
...
export default {
  ...
  methods: {
    ...
    listHeight() {
      //计算每个区间的高度
      let height = this.$refs.currentCity.clientHeight + this.$refs.hotCity.clientHeight
